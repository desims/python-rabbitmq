...
service_health = {"rabbitmq_connected": False, "processing": True, "mongodb_connected": False}
...
try:
    ...
    message_collection = db[COLLECTION_NAME]
    service_health["mongodb_connected"] = True
    logging.info("Connected to MongoDB successfully.")
except errors.ServerSelectionTimeoutError as e:
    logging.critical(f"MongoDB connection error: {e}")
    service_health["mongodb_connected"] = False
...
def process_message(body):
    ...
    try:
        ...
        save_message_to_db(user_message, response_text)

        # Log the response
        logging.info(f"Responded: {response_text}")
...
def callback(channel, method, properties, body):
    ...
    """
    try:
        with ThreadPoolExecutor(max_workers=10) as executor:  # Increased workers for scalability
...
def consume_messages():
    ...
    while True:
        try:
            # Connect to RabbitMQ with a pooled connection
            connection = pika.BlockingConnection(pika.ConnectionParameters(host=RABBITMQ_HOST))
            channel = connection.channel()

            # Declare a durable queue to handle larger volumes
            channel.queue_declare(queue="chat_queue", durable=True)
            channel.queue_bind(exchange=EXCHANGE_NAME, queue="chat_queue")
